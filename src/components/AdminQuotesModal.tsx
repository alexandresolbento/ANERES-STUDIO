...
export function AdminQuotesModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm overflow-y-auto">
      <div className="relative w-full max-w-4xl bg-zinc-950 border border-zinc-800 rounded-3xl p-6 sm:p-8 text-zinc-100 shadow-2xl my-8">
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-6 right-6 p-2 rounded-full bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-white hover:border-zinc-700 transition-colors cursor-pointer"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 border-b border-zinc-800/80">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="px-2.5 py-1 text-xs font-bold uppercase tracking-wider rounded-md bg-amber-500/10 text-amber-400 border border-amber-500/20">
                Central de Orçamentos & Gmail
              </span>
              <span className="flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold rounded-md bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
        ...
        {emailSuccessMsg && (
          <div className="mt-4 p-3.5 rounded-xl bg-emerald-950/40 border border-emerald-500/30 text-emerald-300 text-sm flex items-center gap-3">
        ...
        {emailErrorMsg && (
          <div className="mt-4 p-3.5 rounded-xl bg-rose-950/40 border border-rose-500/30 text-rose-300 text-sm flex items-center gap-3">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mt-6 mb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-zinc-400 font-medium">Filtrar:</span>
          ...
          <button
            onClick={loadQuotes}
            disabled={loading}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-xl bg-zinc-900 border border-zinc-800 text-zinc-300 hover:text-white hover:border-zinc-700 transition-all cursor-pointer"
          >
        ...
        <div className="space-y-3.5 max-h-[50vh] overflow-y-auto pr-1">
          {loading && quotes.length === 0 ? (
            ...
            <div className="py-12 text-center text-zinc-500 text-sm border border-dashed border-zinc-800 rounded-2xl">
            ...
            filteredQuotes.map((quote) => {
              ...
              return (
                <div
                  key={quote.id}
                  className="p-4 sm:p-5 rounded-2xl bg-zinc-900/80 border border-zinc-800/80 hover:border-zinc-700/80 transition-all"
                >
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-3">
                    <div>
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        <span className="font-bold text-base text-white">{quote.name}</span>
                        {quote.company && (
                        ...
                        {quote.emailSent ? (
                          <span className="text-[10px] font-semibold text-emerald-400 bg-emerald-950/40 border border-emerald-500/20 px-2 py-0.5 rounded-full flex items-center gap-1">
                            <CheckCircle2 className="w-2.5 h-2.5" /> Email Enviado
                          </span>
                        ) : (
                          <span className="text-[10px] font-semibold text-zinc-500 bg-zinc-800/60 px-2 py-0.5 rounded-full flex items-center gap-1">
                      ...
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-zinc-400">
                        <span>
                          <strong>WhatsApp:</strong> {quote.phone}
                        </span>
                        {quote.email && (
                          <span>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-zinc-950 text-xs font-bold transition-all shadow-sm cursor-pointer disabled:opacity-50"
                      >
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="flex items-center gap-1 px-3 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold transition-all"
                      >
                  ...
                  {quote.message && (
                    <div className="p-2.5 rounded-xl bg-zinc-950/60 border border-zinc-800/60 text-xs text-zinc-300 italic mb-3">
                      "{quote.message}"
                    </div>
                  )}
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-zinc-800/50 text-[11px] text-zinc-500">
                    <div>
                      ID: <span className="font-mono text-zinc-400">{quote.id}</span> • Registrado em:{' '}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <select
                        ...
                        className="bg-zinc-800 border border-zinc-700 text-zinc-200 rounded-lg px-2 py-0.5 text-xs focus:outline-none focus:border-amber-500"
                      >
        ...
        {selectedQuoteForEmail && (
          <div className="fixed inset-0 z-60 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md">
            <div className="w-full max-w-lg bg-zinc-900 border border-amber-500/30 rounded-3xl p-6 sm:p-7 shadow-2xl text-zinc-100">
              ...
              <p className="text-sm text-zinc-300 mb-4 leading-relaxed">
                ...
                <strong className="text-amber-400">{selectedQuoteForEmail.name}</strong> para{' '}
                <strong className="text-white">{ADMIN_EMAIL}</strong>
                {selectedQuoteForEmail.email && (
                  <>
                    {' '}
                    e com cópia para o solicitante (
                    <strong className="text-white">{selectedQuoteForEmail.email}</strong>)
                  </>
                )}
                .
              </p>

              <div className="p-3.5 bg-zinc-950 rounded-2xl border border-zinc-800 text-xs text-zinc-400 space-y-1 mb-5">
                <div>
              ...
              <div className="flex items-center justify-end gap-3">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-5 py-2 rounded-xl text-xs font-bold text-zinc-950 bg-amber-500 hover:bg-amber-400 transition-all shadow-md cursor-pointer disabled:opacity-50"
                >
